...
impl<const W:usize,const L:usize>  Game<W,L>{
    pub fn new() -> Self{
        ...
        let cell = matrix.index_mut((5,5));
        *cell = CellState::SnakeHead as u8;

        let cell = matrix.index_mut((4,5));
        *cell = CellState::SnakeBody as u8;
    ...
    pub fn reset(&mut self){
        self.score =0;
        self.snake.head = (rand::gen_range(1,W as isize-1),rand::gen_range(1,L as isize-1));
...
impl<const W:usize,const L:usize>  Game<W,L> {
    ...
    fn generate_state(&mut self){
        ...
        *self.state.index_mut((self.snake.head.0 as usize,self.snake.head.1 as usize)) = CellState::SnakeHead as u8;
        for body in self.snake.bodies.iter().map(|a| (a.0 as usize,a.1 as usize)){
...
impl<const W:usize,const L:usize>  Game<W,L>{
    ...
    pub fn draw(&self){
        ...
        for i in 0..(W*L){
            let (x,y)= (i%W,i/L);
            let (pix_x,pix_y) = (padding + (cell_size*gap)*x as f32 ,padding + (cell_size*gap)*y as f32);
